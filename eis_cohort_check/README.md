...
(mean next-cycle capacity MAE 3.13 → 2.20 mAh; median improvement 27%; Wilcoxon signed-rank p = 1.2×10⁻⁷).
A local rerun of the dataset's original forecasting pipeline on the same cells shows the
same direction (point-wise median absolute percentage error 8.3% → 6.0%, pooled R² 0.81 → 0.91); the target throughout is the next-cycle discharge capacity, the same quantity the original study rolls forward into lifetime forecasts.
...
- `deep_suite.py` — the four-arm pilot (thesis Fig. 6.7): capacity / EIS-only / combined /
  ...
  structured cell-group holdout (PJ097–112 ↔ PJ145–152, consecutive identifier blocks within the same cohort). Key results: EIS alone is *worse*
  than capacity features alone (complement, not replacement); 1 frequency ≈ 71% and
  3 ≈ 79% of the full-spectrum gain (most-picked 6.5–7.4 Hz); the cell-group-shift penalty
